...
function handleAbdmError(res: any, err: unknown) {
  if (err instanceof AbdmNotImplementedError) {
 ...
 */
router.post('/abha/otp', authMiddleware, async (req, res) => {
 ...
 */
router.post('/abha/verify', authMiddleware, async (req, res) => {
  const { txnId, otp } = req.body as { txnId?: string; otp?: string };
  if (!txnId || !otp) return res.status(400).json({ message: 'txnId and otp are required' });
 ...
 */
router.post('/abha/create', authMiddleware, async (req, res) => {
  const { userId } = (req as any).user;
  const { txnId, firstName, lastName, email } = req.body as {
    txnId?: string; firstName?: string; lastName?: string; email?: string;
  };
  if (!txnId || !firstName) {
  ...
  try {
    const result = await createAbha(txnId, { firstName, lastName, email });
    ...
    await saveAbdmSession(userId, result.token, null, 600);
 ...
 */
router.get('/abha/profile', authMiddleware, async (req, res) => {
  const { userId } = (req as any).user;

  const session = await getAbdmSession(userId);
  if (!session) {
  ...
  try {
    const profile = await getPatientProfile(session.accessToken);
    res.json({ profile });
  } catch (err) {
    handleAbdmError(res, err);
  }
 ...
 */
router.post('/consent/request', authMiddleware, async (req, res) => {
  const { userId } = (req as any).user;
  const {
    patientAbhaAddress,
    fromDate,
    toDate,
    hiTypes = ['DiagnosticReport', 'Prescription', 'ImmunizationRecord'],
  } = req.body as {
