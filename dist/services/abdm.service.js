...
exports.AbdmNotImplementedError = void 0;
exports.getM2MToken = getM2MToken;
exports.sendAadhaarOtp = sendAadhaarOtp;
exports.verifyAadhaarOtp = verifyAadhaarOtp;
exports.createAbha = createAbha;
exports.abhaExists = abhaExists;
exports.getPatientProfile = getPatientProfile;
exports.requestConsent = requestConsent;
exports.fetchHealthRecords = fetchHealthRecords;
exports.saveAbdmSession = saveAbdmSession;
exports.getAbdmSession = getAbdmSession;
...
async function getM2MToken() {
    if (_m2mCache && Date.now() < _m2mCache.expiresAt - 30000) {
        return _m2mCache.accessToken;
    }
    if (!CLIENT_ID || !CLIENT_SECRET) {
        throw new Error('ABDM credentials not configured. Set ABDM_CLIENT_ID and ABDM_CLIENT_SECRET in .env');
    }
 ...
 */
async function createAbha(txnId, profile) {
    // const token = await getM2MToken();
    // ...
 ...
 */
async function abhaExists(abhaNumber) {
    // const token = await getM2MToken();
 ...
 */
async function getPatientProfile(userAbdmToken) {
 ...
 */
async function fetchHealthRecords(consentArtefactId) {
    // const token = await getM2MToken();
    // ...
...
async function saveAbdmSession(userId, accessToken, refreshToken, expiresInSeconds) {
    const expiresAt = new Date(Date.now() + expiresInSeconds * 1000);
    await prisma_1.prisma.abdmSession.upsert({
        where: { userId },
...
async function getAbdmSession(userId) {
    const session = await prisma_1.prisma.abdmSession.findUnique({ where: { userId } });
    if (!session || session.expiresAt < new Date())
        return null;
    return session;
}
// ── Utilities ─────────────────────────────────────────────────────────────────
function abdmHeaders(token) {
    return {
...
async function checkResponse(res) {
    ...
    if (!res.ok) {
        const msg = body?.message ?? body?.details ?? res.statusText;
        throw new Error(`ABDM API error ${res.status}: ${msg}`);
    }
    return body;
}
class AbdmNotImplementedError extends Error {
    constructor(method) {
        super(`ABDM integration not yet active: '${method}' is a stub. ` +
...
exports.AbdmNotImplementedError = AbdmNotImplementedError;
